...
public class DAORol {
    ...
    public static int obtenerIdDirector(int idUsuario) throws SQLException{
        int idDirector = 0;
        ModConexionBD abrirConexion = new ModConexionBD();
        Connection conexion = abrirConexion.getConnection();   
        if (conexion != null){
            try {
                String consulta = "SELECT directordetrabajo.idDirectorDeTrabajo AS IdDirector FROM usuario "+
                        "INNER JOIN directordetrabajo on usuario.idUsuario=directordetrabajo.Usuario_idUsuario "+
                        "WHERE idUsuario=?; ";
                PreparedStatement prepararSentencia = conexion.prepareStatement(consulta);
                prepararSentencia.setInt(1, idUsuario);
                ResultSet resultado = prepararSentencia.executeQuery();
                if(!resultado.next()){
                   ...
                   idDirector = resultado.getInt("idDirector");
                }
                conexion.close();
            } catch (SQLException ex) {
                Utilidades.mostrarDialogoSimple("ERROR DE CONSULRA", "Hubo un error al realizar la consulta", Alert.AlertType.WARNING);
            }
        } else{
            Utilidades.mostrarDialogoSimple("ERROR DE CONEXION", "Hubo un error de conexion ", Alert.AlertType.WARNING);
        }
        return idDirector;
    }
}
